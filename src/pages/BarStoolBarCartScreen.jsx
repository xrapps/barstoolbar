...
import BarStoolBarCartItemComponent from '../components/BarStoolBarCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'BarStoolBarCartSuccessScreen'
      : 'BarStoolBarHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <BarStoolBarCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
